...
import s from "./EditTeam.module.css";
import { SubmitHandler, useForm } from "react-hook-form";
import { Link, useNavigate } from "react-router-dom";
import { ITeam, ITeamFormData } from "../Interfaces/Interfaces";
...
export default function EditTeam({ teamInfo }: editTeamProps) {
    ...
    if (imgData.has("file")) {
      await post("/Image/SaveImage", imgData, token).then(
        (response: string): void => {
          teamData.imageUrl = response;
        }
      );
    } else {
      teamData.imageUrl = teamInfo.imageUrl;
    }

    await put("/Team/Update", JSON.stringify(teamData), token);
    navigate("/teams");
  };
  ...
  useEffect(() => {
    if (imageLabel.current !== null) {
      imageLabel.current.style.backgroundImage = `url(http://dev.trainee.dex-it.ru${teamInfo.imageUrl})`;
    }
  }, []);
  return (
    <div className={s.container}>
      <div className={s.header}>
        <p>
          ...
          <Link to={`/teams/${teamInfo.id}/edit`}>Edit</Link>
        </p>
      </div>
